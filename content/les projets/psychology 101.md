...
1. we tend to find stories and patterns even in sparse data
2. we fill in characteristics from stereotypes, generalities, prior histories
3. we imagine things and people we are familiar with or fond of as better than things and people we are not familiar with or fond of
...
2. in order to stay focused, we favour the immediate, relatable thing in front of us over the delayed and distant
3. in order to get anything done, we are motivated to complete things that we have already invested time and energy in
4. in order to avoid mistakes, we are motivated to preserve our autonomy and status in a group, 
5. and to avoid irreversible decisions, we favour options that appear simple or that have more complete information over more complex, ambiguous options 
...
4. we **simplify probabilities and numbers** to make them easier to think about
	...
	- [[#Money illusion]]
	- [[#Conservatism]]
5. we think we know **what other people think**
	- [[#Curse of knowledge]]
...
1. in order to act, we need to be **confident** in our ability to make an impact and to feel like what we do is important 
	...
	- [[#Hard-easy effect]]
	- [[#Illusory superiority (Lake Wobegone effect)]]
	- [[#Self-serving bias]]
	- [[#Actor-observer bias]]
...
5. and to avoid irreversible decisions, we favour options that appear **simple** or that have more **complete information** over more complex, ambiguous options 
	...
	- [[#Sapir-Whorf-Korzybski hypothesis (Linguistic Relativity)]]
...
1. Information overload -> Filter out most of it => **We don’t see everything.** 
   *Some of the information we filter out is actually useful and important.*
2. Lack of meaning -> Fill in the gaps => **Can generate illusions.**
   *Imagination, assumptions, false meaning, interpretations, stories.*
3. Need to act fast -> Jump to conclusions => **Can be seriously flawed.**
   *Quick decisions can be unfair, self-serving, and counter-productive.*
4. Limited memory capabilities -> Make trade-offs => **Our memory reinforces errors.**
...
# list en vrac (~200)
### Abilene paradox
def: when a group collectively decides on a course of action that no individual member truly desires, due to failure to communicate objections
-> e.g. group of friends agrees to go to restaurant X while nobody actually wants this, but everyone thought others do 
### Absent-mindedness
def: lapse in attention that leads to memory failure, often due to automaticity or divided attention
-> e.g. walk into a walk to get your keys but immediately forget why you're there because you were thinking about something else
### Actor-observer bias
def: explaining one's own behaviour by situational factors but others' behaviour by their personality (-> a form of [[#Fundamental attribution error]] applied to self vs. others)
-> e.g. you are late -> "the traffic was horrible" (external, situation) vs. someone else is late -> because they are "irresponsible" (personality, internal)

cf. [[#Self-serving bias]] or [[#Ultimate attribution error]]
### Ambiguity bias
def: tendency to prefer known risks over unknown risks, even if the known risk is objectively larger
-> e.g. bet with known 50% to win 100€ >>> unknown probability to win the same 100€ even tho it could be between 0-100% 
### Anchoring
...
### Appeal to novelty
def: argument that something is better or more correct simply because it's new, modern or innovative
-> e.g. "we should replace our reliable software with this new app because it's the latest thing, must be better since it's newer"
### Appeal to probability fallacy
...
### Armchair fallacy
def: being overconfident in criticising others' actions or decisions, even if we are not in a position to actually criticise them
-> e.g. watching a historical documentary and thinking "yeah that general was an idiot for that decision", ignoring the actual circumstances of that situation (and also judging in hindsight, from the comfort of your armchair...) 
### Attentional bias
...
-> e.g. 
### Backfire effect
def: when presented with evidence that contradicts a deeply help belief, one may reject the evidence and become even more convinced of their original belief 
-> e.g. sb believes a conspiracy theory doubles down on their belief when shown factual articles debunking it, claiming that the articles are fake or wrong
### Bandwagon effect
...
### Barnum effect (Forer effect)
def: tendency to accept vague, general personality descriptions as uniquely accurate for oneself
-> e.g. reading horoscope and thinking this describes you perfectly, => astrology, fortune telling, aura reading and some types of personality tests even! 

very similar to [[#Subjective validation]], but maybe here is specifically for personality descriptions
### Base rate fallacy
...
### Belief bias
def: tendency to rely on our pre-existing beliefs to evaluate a conclusion, which leads to overestimate the validity of a credible conclusion, independently of its true logical validity 
-> e.g. all birds can fly (belief), penguins are birds (belief too) => penguins can fly (ccl), which is false but it feels intuitively true
### Bias blind spot
...
### Bike-shedding effect (Parkinson's Law of Triviality)
def: tendency for a group to give disproportionate weight to trivial issues while avoiding complex important ones
-> e.g. company meeting spending 45 min debating font style vs. 5 min on annual budget... the reason is because it's easier to do this 
### Bizarreness effect
...
but very much the same as [[#Peak–end rule]] or [[#Duration neglect]]
### Cheerleader effect
...
### Chesterton’s fence
def: a principle that states "Do not remove a fence until you know why it was put up in the first place." i.e. understand the state of things before you act
-> e.g. removing some ugly and seemingly useless protections without considering they actually served a crucial purpose
### Choice-supportive bias
...
### Conjunction fallacy
def: mistaken belief that a specific, detailed scenario (combining two events) is more probable than a single, more general one
-> e.g. judging that it's more likely that "kevin is swe and likes plants" than just "kevin is a swe" (idk)
### Conservatism
...
### Cryptomnesia
def: memory bias where a forgotten memory returns without being recognised as a memory, and is mistaken as a new idea or thought
-> e.g. musician unknowingly recreate a melody they heard before, believing it is an entirely new and orignal composition
### Cue-dependent forgetting
...
### Decoy effect
def: phenomenon where consumers change their preference between two options when presented with a third (= the decoy)
-> e.g. choosing the M menu instead of S or L seems more reasonable in terms of price and size
### Defensive attribution hypothesis
def: attribute blame and root causes to victims of misfortune that might happen to you too, to reassure yourself it won't happen to you
-> e.g. colleague is laid off, you think "it must be because they have been performing poorly" to distance yourself from the threat of being laid off too
### Delmore effect
def: prioritise goals that are easier to articulate and to measure over goals that may be more important but are more vague
-> e.g. "read 10 pages a day" (easy to formulate + track) >> "develop a deeper understanding of the subject" which is more important but more difficult to frame and evaluate
### Denomination effect
def: tendency to spend money more readily when it's in the form of small denominations
-> e.g. more likely to spend 10x 5$ than you are to spend a single 50$ bill on the same kind of purchase, even though it's the same amount 
### Disposition effect
def: tendency of investors to sell assets that have increased in value too early and hold onto assets that have decreased in value for too long
-> e.g. "lock in gains" vs. hope it'll recover, bounce back
### Distinction bias
...
### Dunning-Kruger effect
def: bias where people with low ability at a task overestimate their ability while experts may underestimate theirs 
-> e.g. novice chess player believes they could compete in a tournament and win while not even understanding the game's depth yet
### Duration neglect
def: tendency to ignore or give little weight to the length of an experience when evaluating it, focusing instead on its [[#Peak–end rule|peak and end]] 
-> e.g. 3-minute unpleasant sound is rather as worse than a longer 5-minute one that ends with a gentler note, the emphasis being on the duration aspect

similar to [[#Bucket error (Extension Neglect)]]
### Effort justification
def: tendency to attribute greater value to an outcome that one has put significant effort into obtaining
-> e.g. you value a piece of furniture you spent days assembling > identical, pre-assembled one you could have bought (exactly [[#IKEA effect]] example btw)
### Egocentric bias
def: tendency to rely too heavily on one's own perspective when interpreting events or estimating others' viewpoints 
-> e.g. after a group project, you recall your own contributions more vividly + rate them as more significant vs. those of teammates
### Empathy gap
...
### Endowment effect
def: valuing something *you own* more highly than an identical item you do not own, simply because you own it 
-> e.g. you believe your used car is worth a lot more than what you would be willing to pay for that exact same car from someone else 
### Essentialism
...
### Fading affect bias
def: tendency for the emotions associated with negative memories to fade more quickly than emotions linked to positive memories 
-> e.g. recall happiness of a past birthday party clearly, but the intense anger from an old argument feels distant and less sharp 

vs. [[#Negativity bias]] 
### False consensus effect
def: tendency to overestimate the extent to which others share one's beliefs, attitudes, and behaviours 
-> e.g. a fan of a niche music genre convinced most people also know their favourite bands 
### False memory
def: recollect an event that didn't actually occur, or remembering it very differently from how it happened 
-> e.g. vividly "remembering" getting lost in a mall, after hearing people talk about this, even though this never happened to you
### Focusing effect
...
### Fundamental attribution error
def: overemphasize personality-based explanations for others' behaviour while underemphasizing situational influences
-> e.g. a stranger trips, you assume they are dumb and clumsy (internal) rather than considering the uneven pavement (external)

cf. [[#Actor-observer bias]] or [[#Self-serving bias]] or [[#Ultimate attribution error]]
### Gambler’s fallacy
...
### Generation effect
def: information that you generate yourself is better remembered than information you simply read or hear
-> e.g. it's easier to remember a mnemonics you come up with yourself, than simply read and try to absorb regular ones
### Google effect (Digital amnesia)
...
-> e.g. you don't remember the dates of WWII, all good, "just google it", or simply something else that is digitally accessible, like your friends' addresses 
...
### Hard-easy effect
def: overconfidence on difficult tasks, and underconfidence on easy tasks
-> e.g. more confident to pass the practical driving test vs. theoretical writing test, even though the first one is arguably more difficult for requires both knowledge and skill, while latter only requires knowledge 
### Hindsight bias
...
### Hippo problem
def: tendency for a HIPPO (highest paid person opinion) to carry more weight in a decision
-> e.g. company launches a flawed product despite market research, engineering reports indicating it would fail, but just become the CEO liked the idea
### Hofstadter’s law
...
### Hyperbolic discounting
def: prefer a smaller, sooner rewards over a larger, later reward... and the inverse when rewards are closer to each other in time
-> e.g. you prefer 50$ today > 100$ in a year 
but also you prefer 100$ in 6 years > 50$ in 5 years (pretty similar in time)
### Identifiable victim effect
def: tendency to offer greater aid or feel more empathy when a specific, identifiable person is at risk than for a large, statistical group
-> e.g. donate generously to help a single sick child features in a news story, while ignoring broader charities that could help thousands with the same funds 
### IKEA effect
def: valuing things more highly because you partially created them or put effort into assembling them
-> e.g. you value that wobbly bookshelf you assembled more than a sturdier, pre-assembled one that costs the same 

specific to furniture here, but otherwise similar to [[#Effort justification]] which is more generic 
### Illusion of asymmetric insight
...
### Illusion of control
def: overestimating one's ability to control or influence events that are largely determined by chance
-> e.g. lottery player feels they have a "system" for picking winning numbers like using family birthdays, believing it increases their odds 
### Illusion of external agency
...
### Illusory superiority (Lake Wobegone effect)
def: tendency to rate oneself as above average in desirable traits or abilities 
-> e.g. in survey, most drivers rate their driving skill as above average, which is statistically impossible 
### Illusory truth effect
...
### Implicit associations
def: unconscious automatic link the mind makes between concepts (objects, people or ideas) which can influence attitudes / behaviours
-> e.g. someone might implicitly associate "science" more strongly with "male" than "female", which can affect their judgments despite their conscious beliefs
### Implicit stereotypes
def: automatic application of stereotypical attributes (race, gender, age) to an individual, without conscious awareness
-> e.g. despite believing in equality, one might still unconsciously view a man as more assertive than a woman 
### In-group bias
...
### Information bias
def: tendency to seek info even when it cannot affect action or is irrelevant to the decision, just because it creates an illusion of control or certainty
-> e.g. research every possible detail about minor medical symptom online, causing high stress and anxiety, even though you could just call a doctor and confirm 
### Insensitivity to sample size
...
### Irrational escalation (Escalation of commitment)
def: increase investment in a failing course of action to justify prior investments, often to avoid admitting a mistake 
-> e.g. a company keeps on pouring money into a failing product to try to make back its initial investment, rather than cuttings its losses
### Just-world hypothesis
...
### Law of the instrument (Maslow’s hammer)
def: over-reliance on a familiar tool / method, ignoring more suitable alternatives 
-> e.g. "if you have a hammer, everything looks like a nail"
-> e.g. marketing analyst tries to solve every business pb with a survey, even when others methods are more suitable such as financial analysis or product redesign
*other names: Golden hammer, Law of the hammer*
### Less-is-better effect
def: situational reversal of preference when a smaller, lower-quality item is preferred over a larger, higher-quality one when evaluated separately (but not together)
-> e.g. you receive a gift and you're like "that's sick love that so much", then you receive another gift later, that is more expensive even "hm ok, well seems pretty meh"
=> perception is context-dependent
### Leveling and sharpening
def: memory distortions where details are lost (leveling) and certain features are exaggerated (sharpening) to fit a schema or narrative over time
-> e.g. remembering a childhood visit to a theme park as "perfectly sunny" (sharpening one feature) while forgetting the long lines (leveling other detail)
### Levels of processing effect
def: memory strength is determined by the depth of mental processing, ie deeper, semantic analysis leads to stronger memories than shallow, sensory analysis
-> e.g. you remember a word better if you think about its meaning (semantic) than if you just notice its font or colour (shallow, sensory)
### List-length effect
def: percentage of items correctly recalled from a list decreases as the length of the list increases
-> e.g. you can recall 80% of a 10-word list = 8 words, vs. 40% of a 30-word list = 12 words, so lower percentages but higher absolute counts! 
### Loss aversion
def: psychological pain of losing is 2x as powerful as the pleasure of gaining an equivalent amount, thus people prefer avoiding losses to acquiring gains 
-> e.g. the idea of losing 100$ would make you work wayyyy harder than the idea of gaining 100$... finding 10\$ in the streets is oh well yay but accidentally losing 10\$ is SUCH A DISASTER => the concept of losing is simply 2x more intense in terms of emotional impact 
### Magic number 7+-2
...
### Memory inhibition
def: process by which the activation of one memory can actively suppress the recall of related competing memories
-> e.g. trying to remember a former colleague's name might be harder if you have just been thinking about a current colleague with a similar name 
### Mental accounting
...
### Misattribution of memory
def: correctly remembering an event / fact but incorrectly attributing it to the wrong source / time / place / person 
-> e.g. you remember a funny story a friend told you, but you actually read it online 

very much linked to [[#Source confusion]]
### Misinformation effect
def: recalling an event with less accuracy due to exposure to misleading information after the event 
-> e.g. witness who initially saw a blue car recalls it was red after hearing someone describe it this way

sounds like [[#Suggestibility]], but not necessarily active communication, can be more passive here
### Modality effect
def: tendency for immediate recall of the final items in a list to be better when the list is heard (auditory) than when it is seen visually 
-> e.g. hearing > reading apparently, cause more natural modality maybe

similar to [[#Picture superiority effect]]
### Money illusion
...
fun to compare with [[#Fading affect bias]]
### Neglect of probability
...
### Next-in-line effect
def: impaired memory for the words / actions of a person immediately preceding you in a sequence, as you focus on preparing your own turn 
-> e.g. in a round of introductions, you often forget the name of the person who spoke right before you because you were rehearsing your own introduction
### Normalcy bias
...
### Occam’s razor (Law of parsimony)
def: (problem-solving principle) among competing hypotheses, the one with the fewest assumptions should be selected i.e. simpler explanation > complex one
-> e.g. "when you hear hoofbeats, think horses, not zebras"
### Omission bias
...
### Optimism bias
def: tendency to believe that one is more likely to experience positive events than others 
-> e.g. a smoker acknowledges that smoking causes cancer but believes they are personally less likely to get it than the average smoker
### Ostrich effect
...
### Overconfidence effect
def: general overestimation of one's own abilities, knowledge, control or chance of success 
-> e.g. new driver with only a few weeks of experience, extremely confident they can handle driving in a sever snowstorm
### Pareidolia
...
### Part-list cueing effect
def: providing a subset of items from a memorised list, as cues, hinders the recall of the remaining items, which can seem counter-intuitive
-> e.g. if you're supposed to remember a full list of fruits, and you're given cues "apple, banana" you might inhibit the recall of the rest of the list "orange, grape" vs. if no hint was given at all

related to [[#Memory inhibition]]
### Peak–end rule
def: tendency to judge an experience almost entirely based on how it felt at its peak (most intense point) and at its end, rather than the total or average of every moment
-> e.g. painful medical procedure not being too bad if the final few minutes are less intense, even if the total pain was high 

def similar to [[#Bucket error (Extension Neglect)]]
or [[#Duration neglect]] too 
### Pessimism bias
...
### Picture superiority effect
def: tendency for pics and images to be remembered better than words (the modality is important)
...
-> e.g. easier to remember a logo or diagram than bullet points in a presentation

something similar [[#Modality effect]]
### Placebo effect
...
### Prejudice
def: preconceived opinion or feeling, usually negative, about a person / group that is not based on reason or actual experience
-> e.g. disliking members of a particular religion without ever having interacted with anyone from that group before
### Primacy effect
def: tendency to better remember items presented at the beginning of a list 
-> e.g. in a list of 15 words, you're more likely to remember the first 3-4 words than the rest

and its end too, with [[#Recency effect]]
recency + primacy = [[#Serial position effect]]
### Pro-innovation bias
...
### Processing difficulty effect
def: information that is harder to process is often better remembered, as the brain engages more deeply
-> e.g. study material written in a slightly unusual font can improve your recall of it compared to material in a standard and easy-to-read font
### Projection bias
...
### Pseudocertainty effect
def: perceive an outcome as certain when it is actually uncertain, based on how choices are framed in multi-stage scenarios 
-> e.g. 2x 50% chance to win 30€ >>> 1x 50% chance de win 45€ because the first one "feels" more certain 
### Publication bias
...
### Reactance
def: urge to do the opposite of what someone tries to make you do, in order to protect your sense of freedom and autonomy
-> e.g. mom says not to wear crop tops, so i'll wear crop tops
### Reactive devaluation
...
### Recency effect
def: tendency to better remember and recall what's presented to you recently vs. earlier
-> e.g. in a list of 15 words, you remember the last words better than the ones in the middle 

and for the first items of a list, [[#Primacy effect]]
primacy + recency = [[#Serial position effect]]
### Recency illusion
def: tendency to overestimate the importance of recent events, believing that something you've just noticed is a recent invention, when it has actually been around for a long time
...
### Reverse psychology
def: persuasive technique that involves advocating for a belief  or behaviour that is opposite to the one desired, with the expectation it'll provoke [[#Reactance]] or the desired outcome
-> e.g. mom says "i bet you cannot keep your room clean for more than 1 week" hoping you'll prove her wrong
### Rhyme as reason effect
def: tendency to perceive rhyming statements as more truthful or accurate than non-rhyming ones
-> e.g. "an apple a day keeps the doctor away" >> "eating fruit daily is good for your health" even though first one is not a valid medical advice 
### Risk compensation (Peltzman effect)
def: tendency to take greater risks when perceived safety increases, potentially offsetting the intended safety benefits
-> e.g. drivers with anti-lock brakes may drive faster, negating some of the safety gains from the new technology 
### Rosy retrospection
...
### Sapir-Whorf-Korzybski hypothesis (Linguistic Relativity)
def: the structure of a language affects its speakers' worldview or cognition
-> e.g. a language with many distincts words for "snow" or "wind" might influence its speakers to perceive these as more distinct categories
### Selective perception
...
### Self-serving bias
def: attribute one's successes to internal factors (skill, effort) and failures to external factors (bad luck, difficulty)
-> e.g. after acing an exam -> "i am so smart" vs. failing -> "the questions were terrible"

sounds similar to [[#Ultimate attribution error]] but for oneself, also similar to [[#Actor-observer bias]] and [[#Fundamental attribution error]]
### Semmelweis reflex
...
### Serial position effect
def: [[#Primacy effect]] + [[#Recency effect]] creates a U-shape curve for recall probability across a list
-> e.g. one tends to remember the start and the end of a list, vs. what's in the middle
### Serial recall effect
def: recall of items is better when they must be remembered in the exact order they were presented
-> e.g. easier to remember the alphabet in its order than inverse order
### Social comparison bias
def: tendency to dislike and unfairly rate competitors, esp. when they are seen as a threat to one's own self-eval or status
-> e.g. a hiring manager favouring a less-qualified candidate cause the highly skilled one makes them feel insecure
### Social desirability bias
def: tendency to respond to questions in a manner that will be viewed favourably by others, often by over-reporting "good" behaviour 
-> e.g. in a survey, ppl over-report how often they exercise, recycle or vote and under-report habits like excessive drinking 
### Source confusion
def: specific type of [[#Misattribution of memory|misattribution]] where you *forget* the true origin (source) of a memory
-> e.g. you think you saw a gun during a crime scene, while in fact you only believed that because someone told you about that later on 
### Spacing effect
def: psychological phenomenon where information is better remembered if learning sessions are spaced out over time rather than crammed 
-> e.g. Spaced repetition systems (SRS) to study a language is a great way to remember the concepts/words effectively and quickly; 30 min a day during a month >>> 15hours during a single weekend
### Spotlight effect
...
### Status quo bias
def: preference for the current state of affairs, viewing any change from the baseline as a loss
-> e.g. automatically renew the same phone subscription even though there are better options on the market, but this one just feels like the default
### Stereotypical bias
def: act of judging an individual based on the perceived characteristics of the group to which they belong 
-> e.g. assuming an elderly person isn't a tech-savvy, a tall person must be good at basketball 
### Subadditivity effect
...
-> e.g. def what horoscope is all about, "you sometimes doubt your decisions", well that's true for almost everyone (cf. [[#Barnum effect (Forer effect)]])

### Suffix effect
def: disruption of [[#Recency effect]] when an irrelevant, to-be-ignored item (the suffix) is added to the end of an auditory list
-> e.g. when you hear a list of numbers followed by the irrelevant word "STOP", it will make it harder to remember the final letter than if the list didn't have that final irrelevant word
### Suggestibility
def: quality of being inclined to accept and incorporate misleading information from external sources into one's own memories 
-> e.g. when people tell you "did you see that xxx", then you're more likely to recall that xxx just because you were told that

seems like this one can lead to [[#False memory]]
### Sunk cost fallacy
def: continue a behaviour or endeavour based on previously invested resources (time, money, effort) 
-> e.g. finish a boring movie because you paid for it, "i've already watched an hour of it, might as well", even though your time would be better spent doing something else
### Survivorship bias
...
### System justification
def: tendency to defend, justify existing social/eco/political systems even when these disadvantage them
-> e.g. choosing to justify and rationalise the systems, the status quo, just because you personally managed to make it, so you won't recognise the flaws / issues that others might experience and challenge "I made it even in these circumstances, so you should too, it's part of the game."
### Telescoping effect
...
### Testing effect
def: long-term memory is enhanced when some time is spent retrieving information (via a quiz) rather than just restudying it
-> e.g. flashcards to test yourself > reread notes 
### Third-person effect
def: belief that mass media messages have a greater effect on others than on oneself
-> e.g. thinking violent video games make other ppl more aggressive, but you're immune to such influences
### Time-saving bias
...
### Tip of the tongue phenomenon
def: temporary inability to retrieve a known word / information, accompanied by a feeling that recall is imminent 
-> e.g. trying to remember the name of an actor, "i know it, it starts with a B..." 
### Trait ascription bias
def: tendency to see oneself as relatively variable in behaviour while viewing others as having more predictable and fixed traits
-> e.g. you think "i can adapt to environments, be outgoing at parties but serious at work" vs. my colleague is "just a shy person in all contexts"
### Ultimate attribution error
...
### Unit bias
def: tendency to perceive a single unit of something as the appropriate amount to consume or complete
-> e.g. finishing all the chips in a bag because it's there, or a full can of soda cause it's a unit, not because you're hungry or thirsty
### Isolation effect (Von Restorff effect)
...
### Zero-risk bias
def: tendency to prefer eliminating a small risk over reducing a larger risk 
-> e.g. sounds like local immediate fixes >>> general bigger scope fixes, e.g. taking meds to fix an issue locally at 100% rather than tackling it from the root and mitigate a bigger issue though not 100% fix 
